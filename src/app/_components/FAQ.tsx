                  ...
                  return (
                    <StaggerItem key={faq.question}>
                      <div className="border-b border-neutral-800">
                        <button onClick={() => setActive(open ? -1 : faqIndex)} className="flex w-full items-start gap-6 px-8 py-8 text-left transition hover:bg-neutral-900/40">
                          <motion.div
                            animate={{ borderColor: open ? "rgba(163, 230, 53, 0.5)" : "rgba(38, 38, 38, 1)", color: open ? "#a3e635" : "#ffffff", scale: open ? 1.05 : 1 }}
                            transition={{ duration: 0.3, type: "spring", stiffness: 300 }}
                            className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border bg-gradient-to-b from-neutral-800 to-transparent text-2xl font-semibold"
                          >
                            {String(faqIndex + 1).padStart(2, "0")}
                          </motion.div>
                          <div className="flex-1">
                            <div className="flex items-center justify-between gap-6">
                              <h3 className={`text-xl font-medium transition-colors duration-300 ${open ? "text-lime-300" : "text-white"}`}>{faq.question}</h3>
                            ...
                            <AnimatePresence>
                              {open && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ height: { duration: 0.4, ease: [0.22, 1, 0.36, 1] }, opacity: { duration: 0.3, delay: 0.05 } }} className="overflow-hidden">
                                  <p className="mt-5 text-lg leading-8 text-neutral-300">{faq.answer}</p>
                                </motion.div>
                              )}
                            </AnimatePresence>
                          </div>
